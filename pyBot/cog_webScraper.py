...
from pyBot.LITERALS import *
from pyDiagnostic.DTC import *
from pyWebScraper import Manga4Life, GearVN, CellphoneS
...
class clsCogWebScraper(commands.Cog):
    ...
    async def _MainFunction_Manga4Life_Feed(self):
        ...
        DEM.set_event_status(DEM_EVENT_DISCORD_COG_WEB_SCRAPER_MAINFUNCTION_MANGAFEED_CYCLE_START)
        ...
        ### Log INFO
        DEM.set_event_status(DEM_EVENT_DISCORD_COG_WEB_SCRAPER_MAINFUNCTION_MANGAFEED_CYCLE_END)
    ...
    async def _MainFunction_Notify_before(self):
        ...
        while not self.spider.MangaFeed.logged:
            ...
            if DEM.lookup(DEM_EVENT_WEBSCRAPER_MANGA4LIFE_LOGIN_FAILED):
                DEM.set_event_status(DEM_EVENT_DISCORD_COG_WEB_SCRAPER_MAINFUNCTION_MANGAFEED_CYCLE_CANCEL)
                self._MainFunction_Manga4Life_Feed.stop()
                return
    ...
    async def _MainFunction_SalePrice_Feed(self):
        ...
        DEM.set_event_status(DEM_EVENT_DISCORD_COG_WEB_SCRAPER_MAINFUNCTION_SALEPRICE_CYCLE_START)
        ...
        # Log DBG
        DEM.set_event_status(DEM_EVENT_DISCORD_COG_WEB_SCRAPER_MAINFUNCTION_SALEPRICE_CYCLE_END)
    ...
    def _wishlist_update_add(self, website: str, item_id: str) -> str:
        ...
        # 3. Return reply message
        DEM.set_event_status(DEM_EVENT_DISCORD_COG_WEB_SCRAPER_COMMAND_WISHLIST_UPDATE_ADD)
        return message
    ...
    def _wishlist_update_remove(self, website: str, item_id: str):
        ...
        # 3. Return reply message
        DEM.set_event_status(DEM_EVENT_DISCORD_COG_WEB_SCRAPER_COMMAND_WISHLIST_UPDATE_REMOVE)
        return message
    ...
    async def wishlist_update(
        ...
        if not website in WISHLIST_WEBSITE or not action in WISHLIST_UPDATE_ACTION:
            ...
            DEM.set_event_status(DEM_EVENT_DISCORD_COG_WEB_SCRAPER_COMMAND_WISHLIST_UPDATE_UNSUPPORTED)
    ...
    async def wishlist(self, interaction: Interaction, action: str):
        ...
        if action == WISHLIST_ACTION[ELEMENT_INDEX_FIRST]:
            ...
            DEM.set_event_status(DEM_EVENT_DISCORD_COG_WEB_SCRAPER_COMMAND_WISHLIST_LIST)

        # 2. If action is 'restart', proceed with 'restart' sequence
        elif action == WISHLIST_ACTION[ELEMENT_INDEX_SECOND]:
            message = COG_WEBSCRAPER_MSG_011
            self._MainFunction_SalePrice_Feed.restart()
            DEM.set_event_status(DEM_EVENT_DISCORD_COG_WEB_SCRAPER_COMMAND_WISHLIST_RESTART)

        # Check whether input action is valid
        else:
            DEM.set_event_status(DEM_EVENT_DISCORD_COG_WEB_SCRAPER_COMMAND_WISHLIST_UNSUPPORTED)
            message = COG_WEBSCRAPER_MSG_007
